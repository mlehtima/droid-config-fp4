...
:: This is simple windows flashing script for Fairphone 4 device
...
echo(
echo This is a Windows flashing script for Fairphone 4 device.
echo(
echo Power on the device in fastboot mode, by doing the following:
echo 1. Turn off your Fairphone 4.
...
echo    the USB cable to your Fairphone 4.
...
for %%d in ( %serialnumbers% ) do (
  ...
  @call :getvar product
  findstr /R "FP4" %tmpflashfile% >NUL 2>NUL
  if not errorlevel 1 (
